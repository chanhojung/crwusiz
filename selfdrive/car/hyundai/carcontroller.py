    ...
    if self.longcontrol and CS.out.cruiseState.enabled and (CS.scc_bus or not self.scc_live):
      if self.frame % 2 == 0:
        ...
        can_sends.extend(hyundaican.create_scc_commands(
          self.packer, int(self.frame / 2), self.scc12_cnt, CC.enabled and CC.longActive, self.accel, jerk,
    ...
    if (self.frame - self.last_button_frame) * DT_CTRL > 0.25:
      ...
      # cruise standstill resume
        ...
        if self.CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
          ...
          for _ in range(20):
            can_sends.append(
              hyundaicanfd.create_buttons(self.packer, self.CP, CS.buttons_counter + 1, Buttons.RES_ACCEL))
          self.last_button_frame = self.frame
